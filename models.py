...
class NearEarthObject:
    ...
    def __str__(self):
        """Return `str(self)`, a user-readable string representation."""
        return (f"NEO {self.fullname} has a diameter of {self.diameter:.3f} km"
                f"and {'is' if self.hazardous else 'is not'} potentially "
                f"hazardous.")
    ...
    def serialize(self):
        """Return an attribute dictionary for NEO.

        It will be added in the classmethod serialize() of the CloseApproach
        class.
        """
...
class CloseApproach:
    ...
    @property
    def time_str(self):
        """Return a representation of this `CloseApproach`'s approach time.
