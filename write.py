...
def write_to_csv(results, filename):
    ...
    with open(filename, 'w') as outfile:
        ...
        for cad in results:
            writer.writerow([datetime_to_str(cad.time), cad.distance,
                            cad.velocity, cad._designation, cad.neo.name,
                            cad.neo.diameter, cad.neo.hazardous])
...
def write_to_json(results, filename):
    ...
    saved.
    """
    r = [cad.serialize() for cad in results]
    with open(filename, 'w') as outfile:
